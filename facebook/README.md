# Facebook Page — Gia Sư Tinh Hoa

> **Page:** https://www.facebook.com/giasutinhhoa.vn
> **Cập nhật:** 2026-09-08

---

## Danh sách bài đăng

| # | Thư mục | Chủ đề | Ghi chú |
|---|---------|--------|---------|
| 01 | [01_gioi_thieu_trang](./01_gioi_thieu_trang/) | Giới thiệu trang | 📌 Pin lên đầu |
| 02 | [02_tuyen_gia_su](./02_tuyen_gia_su/) | Tuyển gia sư | Target: Sinh viên, giáo viên |
| 03 | [03_thu_hut_phu_huynh](./03_thu_hut_phu_huynh/) | Thu hút phụ huynh | Target: Phụ huynh |
| 04 | [04_meo_hoc_tap](./04_meo_hoc_tap/) | Mẹo học tập | Content giá trị |
| 05 | [05_gia_su_noi_bat](./05_gia_su_noi_bat/) | Gia sư nổi bật tuần | Template — cần data thực |
| 06 | [06_tan_sinh_vien_lam_gia_su](./06_tan_sinh_vien_lam_gia_su/) | Tân sinh viên làm gia sư | Target: Sinh viên + Phụ huynh |
| 07 | [07_chuan_bi_nam_hoc_moi](./07_chuan_bi_nam_hoc_moi/) | Chuẩn bị năm học mới | Target: Phụ huynh — Seasonal |
| 08 | [08_loi_ich_su_dung](./08_loi_ich_su_dung/) | Lợi ích khi sử dụng Gia Sư Tinh Hoa | Target: Phụ huynh + Gia sư — Evergreen |
| 09 | [09_dau_hieu_ngop_nam_hoc_moi](./09_dau_hieu_ngop_nam_hoc_moi/) | 3 Dấu hiệu con bị ngợp đầu năm học | Target: Phụ huynh chuyển cấp — Seasonal |

---

## Cấu trúc thư mục

```
facebook/
├── README.md              ← File này
├── CHATBOT_SETUP.md       ← Hướng dẫn setup chatbot
├── 0_cover.png            ← Ảnh bìa Facebook Page
│
├── 01_gioi_thieu_trang/
│   ├── CONTENT.md         ← Nội dung + ghi chú
│   └── hinh_anh.png       ← Ảnh đăng kèm
│
├── 02_tuyen_gia_su/
│   ├── CONTENT.md
│   └── hinh_anh.png
│
├── 03_thu_hut_phu_huynh/
│   ├── CONTENT.md
│   └── hinh_anh.png
│
├── 04_meo_hoc_tap/
│   ├── CONTENT.md
│   └── hinh_anh.png
│
└── 05_gia_su_noi_bat/
    └── CONTENT.md          ← Template, chưa có ảnh
```

---

## Quy tắc thêm bài mới

1. Tạo thư mục `06_{{ten_chu_de}}`
2. Thêm `CONTENT.md` — chứa nội dung đăng + ghi chú
3. Thêm `hinh_anh.png` — ảnh đăng kèm
4. Cập nhật bảng danh sách ở README này
