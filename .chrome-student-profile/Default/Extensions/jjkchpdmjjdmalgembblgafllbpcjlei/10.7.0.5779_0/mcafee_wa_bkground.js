...
var mcafee_wa_annotationoptions ={
     Unknown            : -1,
     None               :  1,
     OnlySecureSearch   :  2,
     All                :  4
...
{
    uninstallUrl            : "https://home.trellix.com/root/landingpage.aspx?lpname=get-it-now",
    siteAdvisorUrl          : "http://www.siteadvisor.com/",
    siteAdvisorUrlSecure    : "https://www.siteadvisor.com/",
    siteReportUrl           : "https://trustedsource.org/en/feedback/url?action=checksingle&url=",
    ...
    WCBlockPageUrl          : "https://www.trellix.com/SAE/BlockPageGC.html",
    WCWarnPageUrl           : "http://www.trellix.com/SAE/WarnPromptPageGC.html",
    WCWarnPageUrlSecure     : "https://www.trellix.com/SAE/WarnPromptPageGC.html",
    WCIframeBlockUrl        : "http://www.trellix.com/SAE/subframeblockpage.html",
    WCIframeBlockUrlSecure  : "https://www.trellix.com/SAE/subframeblockpage.html",
...
var mcafee_wa_commands =
{
    ...
    disabledEdge        : 8,
    disabledOpera       : 16
};var mcafee_wa_executecommands = {
	showsettingsdialog 	: 1,
	checkpassword       : 2,	
	raisenewtabevent    : 3,		
	showwelcomemessage  : 4
};
var mcafee_wa_extensionmsgs = {
    setoptionstring     : 1,
    sendtelemetry       : 2,
    ping                : 3,
    syncoptionsfrommain : 4
};var mcafee_wa_extensiontypes= {
	Unknown : -1,
	Main    :  0,
	Search  :  1
...
mcafee_wa_resourcerequestor.prototype.getURI = function (resourceRelativeURI)
{
    ...
    {
        uri = chrome.runtime.getURL(resourceRelativeURI);
    }
    return uri;
}
...
var mcafee_wa_settings = {
    ...
    'debugmode': false,
    'browsertype': mcafee_wa_browsertypes.Firefox,
    'extnversion': '10.7.0.5027',
...
};var mcafee_wa_browseraction = function ()
{
    ...
    {
        if (null != chrome.action && typeof (chrome.action) != 'undefined')
        {
        ...
        if (null != chrome.action && typeof (chrome.action) != 'undefined')
        {
			...
			chrome.tabs.get(tabId,function(tab) {
				if (chrome.runtime.lastError) {
					mcafee_wa_globals.logger.error("This tab does not exist continuing will give chrome.runtime.lasterror");
				} else {
						chrome.action.setIcon({ 'path': imagePath, 'tabId': tabId });
...
mcafee_wa_webrequestlistener.prototype.init = function ()
{
    ...
    chrome.webRequest.onCompleted.addListener(function (details)
    {
		...
		//Check the global array of requestId's to see if the onCompleted event is for a download URL
		var index = mcafee_wa_bkglobals.downloadReqID.indexOf(parseInt(details.requestId));
		if (index !== -1) {				//if the requestId is found, remove from array and return so no block page will be shown for direct download URL
			mcafee_wa_bkglobals.downloadReqID.splice(index, 1);
			return;
		}
	...
	chrome.webRequest.onHeadersReceived.addListener(function (details)
	{
        ...
        try {
			const {
			  ...
			  if(url == "https://www.youtube.com/embed/?enablejsapi=1&rel=0&autoplay=0&playsinline=1&expflag=embeds_enable_muted_autoplay%3Atrue&fs=1"){
				  return;
			  }
			...
			//check the content-type type, if application, do nothing because the downloadEvent will hit
			  for (const { name, value } of responseHeaders)
			  {
				if (name.toLowerCase() === 'content-type' && value.toLowerCase().includes('application')){
					mcafee_wa_bkglobals.downloadReqID.push((parseInt(details.requestId))); 			//in case of download, add requestId to global array and check the array in onCompleted
					return;
				}
			  }

			//if we didn't return that means that it is just URL navigation
			//call onBeforeRequest here and pass it details. This will then call the c++ for URL navigation
			//onBeforeRequest(details);

			mcafee_wa_dispatchwrap.onBeforeNavigate(details.url, details.tabId, details.frameId, details.responseHeaders);
		}
		catch (e) {
			
		}

		}, { urls: ["http://*/*", "https://*/*", "ftp://*/*"], types: ["main_frame", "sub_frame"]} , ["responseHeaders"]);
...
mcafee_wa_tablistener.prototype.init = function ()
{
    ...
    chrome.tabs.onUpdated.addListener(function (tabId, changeInfo, tab)
    {
        ...
        if (changeInfo.status == "complete" && typeof (tab.url) != "undefined")
        {
            ...
            {
                mcafee_wa_dispatchwrap.onNavigate(tab.url, tabId, 0);
            }
...
mcafee_wa_legacynativemsgdispatcher.prototype.addNewContext = function (tabid, requestid, uri, istopmost,current_tab)
{
    if( uri.search(/blockpagegc.html|warnpromptpagegc.html|subframeblockpage.html/i) !== -1 ) return;
    
    this._contextMap[tabid] = {req: requestid, url: uri, topmost: istopmost, refcount: 1,cur_tab: current_tab };
    return tabid;
};
...
mcafee_wa_legacynativemsgdispatcher.prototype.navigateTab = function (tabid, url)
{
	...
	// Getting the frame id from context map, in firefox we have different mechanism because tab_ids follow different pattern in firefox.
	
	var contextInfo = this._contextMap[tabid];
	var frameId = contextInfo.topmost==0;
	var ntabId = contextInfo.cur_tab;
    ...
    if (frameId == 0) // If the message is not intended to a iFrame then navigate the tab to the url specified.
    {
        if (mcafee_wa_bkcommon.isSiteAdvisorURL(url))
        {
            if(mcafee_wa_bkconstants.enterprise == true)
            {
                ...
                {
                    var newTabUrl = "http://www.trellix.com/SAE/";
...
{
    if( imageIndex >=  this._icons.length || imageIndex < 0)
    {
        return null;
    }
...
mcafee_wa_legacynativemsgdispatcher.prototype.setBrowserActionIcon = function (iconIndex, tabId)
{
	 ...
	 console.log(" called 2367");
     if (null != imagePath)
     {
		 
		 // Checking if tab is present then only we are sending message, this would work fine in firefox even if we don't include this if 
		 // but in chrome we should have a listener for any message we send.
...
mcafee_wa_legacynativemsgdispatcher.prototype.changeBrowserAction = function(contextId, iconIndex)
{
	...
	//frameid = 0;
	var contextInfo = this._contextMap[contextId];
	var frameid = contextInfo.topmost==0;
	console.log("change browserAction called");
	console.log(frameid);
    if (frameid==0) // Extension icon only reflects the safety rating of top frame
    {
        //var tabId = this.toTabId(contextId);
        this.setBrowserActionIcon(iconIndex, contextId);
    }
};
...
mcafee_wa_legacynativemsgdispatcher.prototype.processOption = function (optionName, optionValue)
{
	...
	// Quering tabs before sending messages, this would work fine in firefox even if we don't query bcz in firefox it's not compulsary to have a listener for every message
    chrome.tabs.query({}, function (tabs)
    {
        ...
        {
			if (tabs[i].url.match('http://*/*') || tabs[i].url.match('https://*/*')){
...
mcafee_wa_legacynativemsgdispatcher.prototype.getXYViewport = function ()
{
    ...
    var pThis       = this;
	chrome.tabs.get(mcafee_wa_bkglobals.activetabId, function(tab) {
		if (chrome.runtime.lastError) {
			mcafee_wa_globals.logger.error("This tab does not exist continuing will give chrome.runtime.lasterror");
		}
		else
		{
			if (tab.url.match('http://*/*') || tab.url.match('https://*/*')){
...
mcafee_wa_legacynativemsgdispatcher.prototype.listen= function (fncallback)
{
    ...
    this._port.onMessage.addListener(function (params)  //Fired when a message is received from native host process
    {
        ...
        else 
        {
			...
			if (params.length >3)
			{	
				if(params[2]=="changeBitmap" && params[3]==-2147467259)
				{
					for(var l=0;l<pThis._icons.length;l++){
						console.log(pThis._icons[l]);
						if(pThis._icons[l][13]=='e'){
							params[3] = l;
							break;
						}
					}
...
mcafee_wa_legacynativemsgdispatcher.prototype.onBeforeNavigate = function (uri, tabId, frameid, responseHeaders)
{
    ...
    this.addNewContext(contextId, contextId, uri, bTopmost,tabId);
    mcafee_wa_bkglobals.logger.log("onBeforeNavigate event called for ( uri, tabId, frameid, contextid) (" + uri + "," + tabId + "," + frameid + "," + contextId + ")");
	this.nativeMsg([mcafee_wa_nativecommands.onbeforenavigate, uri, tabId.toString(), contextId.toString(), bTopmost]);
    return false;
};
...
mcafee_wa_background.prototype.processActiveTab = function ()
{
    chrome.tabs.query({ 'active': true }, function (tabs)
    {
		...
		else if (!mcafee_wa_bkcommon.isProperWebURL(tabs[0].url))
        {
            var imagePath = mcafee_wa_msgdispatchhelper.toQualifiedImagePath("nb_button_disabled.gif");
            mcafee_wa_browseraction.setIcon(imagePath, tabs[0].id, tabs[0].url);
        }
