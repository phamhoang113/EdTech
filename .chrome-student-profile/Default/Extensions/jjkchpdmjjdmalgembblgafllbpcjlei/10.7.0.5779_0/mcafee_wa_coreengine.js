    ...
    try
	{
       ...
       /* var httpRequest = new XMLHttpRequest();
		...
		.then(data=>{
			...
			/*var final_data = JSON.stringify(data.searchengines);
			var finul = {"searchengines":final_data};
			console.log(finul);
			fnCallback(finul);*/
		})
		.catch(error => {
...
var mcafee_wa_linkharvester = function ()
{
    ...
    var harvestLinksInternal = function ( document, engine, fncallback )
    {
        ...
        try
        {
            ...
            var originalRequestCount = mcafee_wa_globals.reputationReqCount;
            mcafee_wa_globals.reputationReqCount = 1;
            ...
            for( var elemIndex = unProcessedAnchors.length-1; elemIndex >= 0 ; --elemIndex )
            {
                ...
                if (( mcafee_wa_globals.reputationReqCount > 0) && ( requestElements.length == mcafee_wa_globals.reputationReqCount) )
                {
                    ...
                    if(mcafee_wa_globals.reputationReqCount==1)
                    {
                        mcafee_wa_globals.reputationReqCount = originalRequestCount;
                    }else
                    {
                        requestURLs     = [];
                        requestElements = [];
    ...
    for (var catIndex = 0; catIndex < 3; ++catIndex)
    {
        ...
        else if ( mcafee_wa_constants.enterprise == true && annoItem.CCList.length > catIndex)
        {
            ...
            if ( (typeof(CCList) != "undefined") && typeof (CCList[0]) != "undefined")
            {
                ...
                if ('undefined' != typeof (CCList[catIndex]["desc"]) && CCList[catIndex]["desc"].length > 0 )
                {
...
mcafee_wa_annotationinserter.prototype.insertAnnotationImage = function (link, linkinfo, config)
{
	try
	{
	    ...
	    divAnnotation.setAttribute("class", "annotations");
		divAnnotation.setAttribute("show_ballon", "mcafee_wa_scripts:wa_showballoon(event);");
		divAnnotation.setAttribute("hide_ballon", "mcafee_wa_scripts:wa_hideballoon(event);");
...
{
	var onPopupAddition = function(mutationsList) {
	  for (let mutation of mutationsList) {
		if (mutation.type === "childList" && mutation.addedNodes.length > 0) {
		  // Select all pop up elements with cass name as "pop_up" under img tag
		  /*
		  if(ab>=1){
			  break;
		  }
		  
		  const popupElements = document.querySelectorAll("div.annotations>div");
		  var len = popupElements.length;
		  if(len>0){
			  ab++;
		  }
		  // traversing over each pop up element to attach listener
		  popupElements.forEach((popupElement) => {
			  
			// Attaching an event listener to pop up elements
			//var show_ball = popupElement.getAttribute("show_ballon");
			//var hide_ball = popupElement.getAttribute("hide_ballon");
			console.log(popupElement);
			popupElement.addEventListener("mouseover", function(event){
				var toId = event.target.getAttribute('id');
				window.postMessage({ type: 'message', 'id': 1, 'toId': toId }, '*');
			  
			});
			popupElement.addEventListener("mouseout", function (event) {
				var toId = event.relatedTarget.getAttribute('id');
				window.postMessage({ type: 'message', 'id': 2, 'toId': toId }, '*');
				
			});
		  });
		  break;*/
		  mutation.addedNodes.forEach((node) => {
                if (node.nodeType === 1 && node.classList.contains('annotations')) {
                    // Add event listener to the added node
    ...
    var annotateInternal = function (domdoc)
    {
		...
		const anno_mutation_observer = new MutationObserver(onPopupAddition);

        anno_mutation_observer.observe(domdoc.body, { childList: true, subtree: true });
        return true;
    };
